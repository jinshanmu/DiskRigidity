...
theorem circleIntegral_logDeriv_eq_finsum_analyticOrderNatAdd {f : ℂ → ℂ} {c : ℂ} {R : ℝ}
  ...
  rcases hf.exists_finset_eq_prod_smul_nonzero (isCompact_closedBall _ _)
  ...
  · rintro z ⟨hzc, hz⟩
    ...
    have hzw_eq : z = w := sub_eq_zero.mp (eq_zero_of_pow_eq_zero hzw)
    simpa [hzw_eq] using hwt
  · exact ht_sub
  ...
  · obtain ⟨R, hR₀, hRU, hfR⟩ : ∃ R > 0, closedBall c R ⊆ U ∧ ∀ w ∈ sphere c R, f w ≠ 0 := by
    ...
    · replace this := this.resolve_left (by simp)
      ...
      · use c
        ...
        infer_instance
      · have hzero :=
          AnalyticOnNhd.codiscreteWithin_setOfPred_analyticOrderAt_eq_zero_or_top
            ((hfd.analyticOnNhd hUo).mono hRU)
        have := (isCompact_closedBall c R).finite_sdiff_of_mem_codiscreteWithin hzero
...
theorem eqOn_const_or_injOn_of_tendstoLocallyUniformlyOn {ι : Type*} {U : Set ℂ}
  ...
  have hf_sub : TendstoLocallyUniformlyOn (fun i z ↦ F i z - F i y) (f · - f y) l (ball x r) := by
    refine (hf.mono hrU).fun_sub <|
      (hf.tendsto_at hy).tendstoUniformlyOn_const _ |>.tendstoLocallyUniformlyOn
...
theorem UnitDisc.hasDerivWithinAt_shift_comp {f : ℂ → UnitDisc} {z f' : ℂ} {s : Set ℂ}
  ...
  have hden : 1 + conj (w : ℂ) * (f z : ℂ) ≠ 0 := UnitDisc.shift_den_ne_zero w (f z)
  have h := (hf.const_add (w : ℂ)).fun_div
    ((hf.const_mul (conj (w : ℂ))).const_add 1) hden
  have hw : (‖(w : ℂ)‖ : ℂ) ^ 2 = conj (w : ℂ) * (w : ℂ) := by
    simpa [mul_comm] using (Complex.mul_conj' (w : ℂ)).symm
  have hcoeff :
      (((1 : ℂ) - (‖(w : ℂ)‖ : ℂ) ^ 2) /
          ((1 : ℂ) + conj (w : ℂ) * (f z : ℂ)) ^ 2 * f') =
        (f' * ((1 : ℂ) + conj (w : ℂ) * (f z : ℂ)) -
          ((w : ℂ) + (f z : ℂ)) * (conj (w : ℂ) * f')) /
          ((1 : ℂ) + conj (w : ℂ) * (f z : ℂ)) ^ 2 := by
    rw [hw]
    field_simp
    ring
  rw [hcoeff]
  simpa only [UnitDisc.coe_shift] using h
...
theorem exist_map_unitDisc_injOn_norm_deriv_gt {U : Set ℂ} (hUo : IsOpen U)
  ...
  · rcases exists_map_unitDisc_injOn_deriv_ne_zero₀ hUo hUc hU (x := x) with
      ⟨g, hg₀, hg_inj, hdg⟩
  ...
  have hdg : ∀ z ∈ U, HasDerivAt (g · : ℂ → ℂ)
    ...
    intro z hz
    have hnat : HasDerivAt (UnitDisc.coe ∘ g)
        (((1 - ‖((-c : UnitDisc) : ℂ)‖ ^ 2) /
              (1 + conj ((-c : UnitDisc) : ℂ) * (f z : ℂ)) ^ 2 *
            deriv (UnitDisc.coe ∘ f) z) /
          ((2 : ℂ) * (g z : ℂ) ^ (2 - 1))) z :=
      (hasDerivAt_pow 2 _).of_comp_left
        ...
        (by norm_num [Function.comp_apply, hg₀ z hz])
        (.of_forall fun x ↦ congr(UnitDisc.coe $(hgf x)))
    have hcoeff :
        (1 - ‖(c : ℂ)‖ ^ 2) /
              (2 * (g z : ℂ) * (1 - conj (c : ℂ) * (f z : ℂ)) ^ 2) *
            deriv (fun x ↦ (f x : ℂ)) z =
          ((1 - ‖((-c : UnitDisc) : ℂ)‖ ^ 2) /
                (1 + conj ((-c : UnitDisc) : ℂ) * (f z : ℂ)) ^ 2 *
              deriv (UnitDisc.coe ∘ f) z) /
            ((2 : ℂ) * (g z : ℂ) ^ (2 - 1)) := by
      simp only [Function.comp_def, UnitDisc.coe_neg, norm_neg, map_neg, neg_mul,
        Nat.reduceSubDiff, pow_one]
      field_simp [hg₀ z hz]
      ring
    rw [hcoeff]
    simpa only [Function.comp_def] using hnat
