...
This is the unit-disc automorphism API used in the proof of the Riemann mapping theorem.  It is
ported from the Apache-licensed upstream Mathlib pull request
...
def shift (z : 𝔻) : 𝔻 ≃ 𝔻 where
  toFun := shiftFun z
  invFun := shiftFun (-z)
  left_inv := fun w ↦ shiftFun_neg_apply_shiftFun z w
...
/-- The composition of two shifts is a rotation composed with another shift. -/
@[simps! coe]
